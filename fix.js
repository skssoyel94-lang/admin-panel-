const fs = require('fs');
const content = fs.readFileSync('artifacts/smovie-admin/app/index.tsx', 'utf8');

const targetStr = `              {item.poster_path ? <Image source={{ uri: \`\${POSTER_BASE}\${item.poster_path}\` }} style={nd.poster} />             <TouchableOpacity style={wm.addBtn} onPress={onClose}><Text style={wm.addTxt}>Close</Text></TouchableOpacity>
...
const replacementStr = `              {item.poster_path ? <Image source={{ uri: \`\${POSTER_BASE}\${item.poster_path}\` }} style={nd.poster} /> : <View style={[nd.poster, nd.posterFb]}><Text style={{ color: MUTED }}>▣</Text></View>}
              <View style={{ flex: 1 }}>
...
fs.writeFileSync('artifacts/smovie-admin/app/index.tsx', content.replace(targetStr, replacementStr));
